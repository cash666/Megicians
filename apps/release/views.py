...
class ReleaseTaskView(LoginRequiredMixin,View):
	...
	def post(self,request):
		...
		if id and type == 'back':
			...
			result={}
			content=''
			log="/devops/megicians/logs/back_release.log"
			proddir="/lf/code/packages/WAR/"
			war_name="%s%s.war_%s" % (proddir,tomcatname,tagname)
			war_decompression="/lf/code/version/%s/%s" % (tagname,tomcatname)
			if os.path.isfile(war_name):
				if not os.path.exists(war_decompression):
					os.makedirs(war_decompression)
				os.chdir(war_decompression)
				code=subprocess.call('jar xf %s' % war_name,shell=True)
				if code != 0:
					content=u'<strong>war包解压出现错误！</strong>'
                                	result['status']='fail'
                                	result['content']=content
                                	return HttpResponse(json.dumps(result))
					
			else:
				content=u'<strong>war包不存在，请先上传对应的war包！</strong>'
				result['status']='fail'
                                result['content']=content
                                return HttpResponse(json.dumps(result))
			...
			log="/devops/megicians/logs/front_release.log"
			content=''
			result={}
			...
			#		try:
				...
				if front_type == 'node':
                                        try:
                                                r=MyRunner([front_host,])
                                                date=time.strftime("%Y-%m-%d-%H-%M", time.localtime())
                                                r.run('shell','[ -d /usr/local/html/%s ] || mkdir -p /usr/local/html/%s' % (front_project.lower(),front_project.lower()))
                                                r.run('shell','/bin/cp -a /usr/local/html/%s /root/BackUp/sim/%s.%s' % (front_project.lower(),front_project.lower(),date))
                                                r.run('synchronize','src=/lf/code/version/%d_%s/  dest=/usr/local/html/%s/ delete=yes recursive=yes' % (project_version,project_md5,front_project.lower()))
                                                r.run('shell','cd /usr/local/html/%s;bash start.sh sim' % front_project.lower())
                                                ret=r.get_result()
                                        except Exception,e:
                                                content=u'<h2 style="color:red">本次任务发布失败!</h2><br/>'
                                                content+='<hr/>'
                                                content+=u'<strong>发布步骤执行出错，请截图，然后联系管理员曹帅进行解决<strong>'
                                                result['status']='fail'
                                                result['content']=content
						EMAIL_TITLE=u'本次任务发布失败!'
                                        	EMAIL_BODY=u'您本次的任务:%s(项目:%s,项目类型:%s,项目主机:%s,构建号:%s,Revesion:%s)发布失败，请截图，然后联系管理员曹帅进行处理！' % (front_task_obj.task_name,front_project,front_type,front_host,project_version,project_md5)
						if request.user.email:
                                                	EMAIL_TO=[request.user.email,]
                                        	else:
                                                	EMAIL_TO=['caoshuai@lifang.com',]
					...
					try:
                                                r=MyRunner([front_host,])
                                                date=time.strftime("%Y-%m-%d-%H-%M", time.localtime())
                                                r.run('shell','[ -d /usr/local/nginx/html/sim/%s ] || mkdir -p /usr/local/nginx/html/sim/%s' % (front_project.lower(),front_project.lower()))
                                                r.run('shell','/bin/cp -a /usr/local/nginx/html/sim/%s /root/BackUp/sim/%s.%s' % (front_project.lower(),front_project.lower(),date))
                                                r.run('synchronize','src=/lf/code/version/%d_%s/  dest=/usr/local/nginx/html/sim/%s/ delete=yes recursive=yes' % (project_version,project_md5,front_project.lower()))
                                                r.run('shell','chown -R apache:apache /usr/local/nginx/html/sim/')
                                                ret=r.get_result()
                                        except Exception,e:
                                                content=u'<h2 style="color:red">本次任务发布失败!</h2><br/>'
                                                content+='<hr/>'
